import { DescribeInstanceInformationCommand, InstanceInformation, PingStatus, SSMClient } from "@aws-sdk/client-ssm";
...
import { ConfigurationTarget, ExtensionContext, ProgressLocation, Uri, commands, window, workspace } from "vscode";
...
export class OpenCommandProvider implements CommandProvider<CommandSuffix.Open, string> {
    constructor(private serviceFactory: AwsClientFactory, private instanceStore: InstanceStore, private profileStore: Observable<string>, private context: ExtensionContext, private instanceStarter: InstanceStarter, private awsContextResolver: AwsContextResolver) {
    }
    ...
    async requestInstanceInfo(instanceId: string): Promise<InstanceInformation | undefined> {
    ...
    protected async requestUsername(instanceInfo: InstanceInformation | undefined, label: string): Promise<string | undefined> {
