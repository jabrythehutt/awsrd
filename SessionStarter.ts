...
export class SessionStarter {
    constructor(private ssmClient: SSMClient, private sessionManagerBinPath: string) {
    }

    async start(request: { instanceId: string, port: number, }): Promise<ChildProcess> {
        ...
        const response = await this.ssmClient.send(new StartSessionCommand(startSessionParams))
        ...
        process.stdin.pause();
        const child = spawn(this.sessionManagerPath, ssmPluginArgs, { stdio: 'inherit' });
    ...
    get sessionManagerPath(): string {
        ...
        return resolve(this.sessionManagerBinPath, `${platform()}_${architecture}_plugin/session-manager-plugin`);
    }
}