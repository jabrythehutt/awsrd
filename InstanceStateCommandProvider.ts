...
export class InstanceStateCommandProvider<T extends CommandSuffix.Start | CommandSuffix.Stop> implements CommandProvider<T, string> {

    constructor(private commandSuffix: T, private instanceStore: InstanceStore, private instanceStarter: InstanceStarter) {
    }
    ...
    async execute(instanceId: string): Promise<void> {
        ...
        const title = `${this.targetState === "running" ? "Starting" : "Stopping"
            } ${label}`;
        await window.withProgress(
            ...
            async (progress, token) => {
                await this.instanceStarter.requestInstanceState(instanceId, this.targetState);
